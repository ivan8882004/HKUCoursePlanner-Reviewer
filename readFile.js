...
fs.readFile(filePath, 'utf8', (err, data) => {
  ...
  const preregKeyWord = ["Prerequisite: ", "Prerequisites: ", "Pre-requisite: ", "Co-requisite: "]
  ...
  for (let i = 0; i < courses.length; i++) {
    ...
    for (let j = 0; j < exclusiveKeyWord.length; j++) {
      ...
      if (exclusiveIndex !== -1) {
        toProcessList[toProcessList.length - 1].exclusive = courses[i].content.slice(exclusiveIndex + exclusiveKeyWord[j].length)
    ...
    for (let j = 0; j < preregKeyWord.length; j++) {
      ...
      if (preregIndex !== -1) {
        toProcessList[toProcessList.length - 1].prereg = courses[i].content.slice(preregIndex + preregKeyWord[j].length)
        break;
      }
    }
  }
