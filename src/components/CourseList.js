function CourseList({ courseList, setCourseList, index, removeCourseList }) {
  const content = courseList.courses.map((course, subIndex) => {
    const editCourseList = event => {
      setCourseList({
        ...courseList,
        courses: [
          ...courseList.courses.slice(0, subIndex),
          event.target.value,
          ...courseList.courses.slice(subIndex + 1),
        ],
      })
    }
    ...
    return (
      <div key={subIndex} className="InfoList">
        <input
          type="text"
          value={course}
          onChange={event => editCourseList(event)}
          placeholder={'Course ' + (subIndex + 1)}
        />
        <button type="button" onClick={() => removeCourseList(subIndex)}>
          X
        </button>
      </div>
    )
  ...
  return (
    <tr key={index}>
      ...
      <td>
        <button type="button" onClick={removeCourseList}>
          Remove courses list
        </button>
      </td>
    </tr>
