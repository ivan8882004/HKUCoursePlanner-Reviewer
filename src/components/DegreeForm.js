...
function DegreeForm() {
  ...
  const editCourseList = (value, index) => {
    setDegree({
      ...degree,
      courseList: [
        ...degree.courseList.slice(0, index),
        value,
        ...degree.courseList.slice(index + 1),
      ],
    })
  }
  ...
  const editFocus = (value, index) => {
    setDegree({
      ...degree,
      focus: [
        ...degree.focus.slice(0, index),
        value,
        ...degree.focus.slice(index + 1),
      ],
    })
  }
  ...
  const removeLeng = index => {
    setDegree({
      ...degree,
      ug5leng: [
        ...degree.ug5leng.slice(0, index),
        ...degree.ug5leng.slice(index + 1),
      ],
    })
  }
  ...
  const lengList = degree.ug5leng.map((cour, index) => {
    return (
      <div key={index} className="InfoList">
        ...
        <button type="button" onClick={() => removeLeng(index)}>
          X
        </button>
      </div>
    )
  ...
  const coursesForm = degree.courseList.map((item, index) => {
    return (
      <CourseList
        courseList={item}
        setCourseList={value => {
          editCourseList(value, index)
        }}
        index={index}
        removeCourseList={() => removeCourseList(index)}
        key={index}
      />
    )
  })
  ...
  const focusForm = degree.focus.map((item, index) => {
    return (
      <FocusForm
        ...
        key={index}
        index={index}
        setFocus={value => editFocus(value, index)}
        removeFocus={() => removeFocus(index)}
      />
    )
  })
  ...
  return (
    <div className="InfoForm">
      <div className="InfoFormTitle">Add/Edit Degree:</div>
      <div className="InfoFormContent">
        <div className="InfoFormLeft">
          <div className="InfoFormSubTitle">Degrees</div>
          <div>{degreesList}</div>
        </div>
        <form className="InfoFormForm">
          <table>
            <tbody>
              ...
              <tr>
                <td>Degree Leng courses:</td>
                <td>{lengList}</td>
              </tr>
              <tr>
                <td>
                  <button type="button" onClick={() => addLeng('')}>
                    Add leng course
                  </button>
                </td>
              </tr>
              <tr>
                <td>Degree major courses:</td>
              </tr>
              {coursesForm}
              <tr>
                <td>
                  <button
                    type="button"
                    onClick={() =>
                      addCourse({ credit: '', courses: [], type: '' })
                    }>
                    Add major courses list
                  </button>
                </td>
              </tr>
              <tr>
              ...
              <tr>
                <td>
                  <button
                    type="button"
                    onClick={() => addFocus({ name: '', courseList: [] })}>
                    Add focus
                  </button>
                </td>
              </tr>
