...
function MajorMinorForm() {
  ...
  const editCourseList = (value, index) => {
    setMajor({
      ...major,
      courseList: [
        ...major.courseList.slice(0, index),
        value,
        ...major.courseList.slice(index + 1),
      ],
    })
  }
  ...
  const editDoubleCount = (event, index) => {
    setMajor({
      ...major,
      doubleCount: [
        ...major.doubleCount.slice(0, index),
        event.target.value,
        ...major.doubleCount.slice(index + 1),
      ],
    })
  }
  ...
  const courseList = major.courseList.map((item, index) => {
    return (
      <CourseList
        courseList={item}
        setCourseList={value => editCourseList(value, index)}
        index={index}
  ...
  const doubleCount = major.doubleCount.map((item, index) => {
    return (
      <div key={index}>
        ...
        <button type="button" onClick={() => removeDoubleCount(index)}>
          X
        </button>
      </div>
    )
  ...
  return (
    <div className="InfoForm">
      <div className="InfoFormTitle">Add/Edit Major Minor:</div>
      <div className="InfoFormContent">
        <div className="InfoFormLeft">
          <div className="InfoFormSubTitle">Majors</div>
          {majorsList}
          <div className="InfoFormSubTitle">Minors</div>
          {minorsList}
        </div>
        <form>
          <table>
            <tbody>
              ...
              <tr>
                <td>
                  <button type="button" onClick={() => addDoubleCount('')}>
                    Add Double Count
                  </button>
                </td>
              </tr>
